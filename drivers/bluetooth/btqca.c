		...
		struct qca_btsoc_version ver, u8 rom_ver, u16 bid)
{
	...
	if (bid == 0x0)
		snprintf(fwname, max_size, "/*(DEBLOBBED)*/", rom_ver, variant);
	else
		snprintf(fwname, max_size, "/*(DEBLOBBED)*/", rom_ver, variant, bid);
}
...
{
	if (bid == 0x0)
		snprintf(cfg->fwname, sizeof(cfg->fwname), "/*(DEBLOBBED)*/", stem, rom_ver);
	else if (bid & 0xff00)
		snprintf(cfg->fwname, sizeof(cfg->fwname),
			 "/*(DEBLOBBED)*/", stem, rom_ver, bid);
	else
		snprintf(cfg->fwname, sizeof(cfg->fwname),
			 "/*(DEBLOBBED)*/", stem, rom_ver, bid);
}
