...
#define VSC_IMAGE_DIR			/*(DEBLOBBED)*/

#define VSC_CSI_IMAGE_NAME		VSC_IMAGE_DIR "/*(DEBLOBBED)*/"
#define VSC_ACE_IMAGE_NAME_FMT		VSC_IMAGE_DIR "/*(DEBLOBBED)*/"
#define VSC_CFG_IMAGE_NAME_FMT		VSC_IMAGE_DIR "/*(DEBLOBBED)*/"
...
static int vsc_identify_csi_image(struct vsc_fw_loader *fw_loader)
{
	...
	ret = reject_firmware(&image, VSC_CSI_IMAGE_NAME, fw_loader->dev);
	if (ret)
		return ret;
...
static int vsc_identify_ace_image(struct vsc_fw_loader *fw_loader)
{
	...
	ret = reject_firmware(&image, path, fw_loader->dev);
	if (ret)
		return ret;
