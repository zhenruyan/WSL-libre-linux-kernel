...
static struct {
	...
	{ IVPU_HW_IP_37XX, "/*(DEBLOBBED)*/" },
	{ IVPU_HW_IP_37XX, "/*(DEBLOBBED)*/" },
	{ IVPU_HW_IP_40XX, "/*(DEBLOBBED)*/" },
	{ IVPU_HW_IP_40XX, "/*(DEBLOBBED)*/" },
	{ IVPU_HW_IP_50XX, "/*(DEBLOBBED)*/" },
	{ IVPU_HW_IP_50XX, "/*(DEBLOBBED)*/" },
	{ IVPU_HW_IP_60XX, "/*(DEBLOBBED)*/" },
};
...
static int ivpu_fw_request(struct ivpu_device *vdev)
{
	...
	if (ivpu_firmware) {
		ret = reject_firmware(&vdev->fw->file, ivpu_firmware, vdev->drm.dev);
		if (!ret)
	...
	for (i = 0; i < ARRAY_SIZE(fw_names); i++) {
		...
		ret = firmware_reject_nowarn(&vdev->fw->file, fw_names[i].name, vdev->drm.dev);
		if (!ret) {
