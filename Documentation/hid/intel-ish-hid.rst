...
Intel will upstream the Intel-built firmware to the ``/*(DEBLOBBED)*/`` repository, located under the path ``intel/ish/``. For the Lunar Lake platform, the Intel-built ISH firmware will be named ``/*(DEBLOBBED)*/``.
