				  ...
				  struct sdca_fdl_file *fdl_file)
{
	...
	while (parent) {
		if (dev_is_pci(parent)) {
			...
			return kasprintf(GFP_KERNEL, "/*(DEBLOBBED)*/",
					 fdl_file->vendor_id,
	...
	return kasprintf(GFP_KERNEL, "/*(DEBLOBBED)*/", fdl_file->vendor_id,
			 ...
			 struct sdca_fdl_set *set, int file_index)
{
	...
	ret = firmware_reject_nowarn(&firmware, disk_filename, dev);
	kfree(disk_filename);
	if (ret) {
		disk_filename = kasprintf(GFP_KERNEL, "/*(DEBLOBBED)*/",
